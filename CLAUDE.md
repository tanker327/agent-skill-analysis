...
- **`src/schema.ts` is the output contract**: `SkillAnalysisSchema` (zod, strict objects) is the single source of truth — all TS types come from `z.infer`, and the JSON Schema artifact is generated from it at build time (`scripts/generate-schema.mjs` via tsup `onSuccess`) to TWO locations: `dist/skill-analysis.schema.json` (ships in the package) and `skill-analysis.schema.json` (committed at the repo root so contract diffs are reviewable). An unconditional byte-match test fails if the committed copy drifts — after any schema.ts change, run `npm run build` and commit the regenerated root artifact with it. Output-shape changes must update the schema and its conformance tests in the same commit.
...
- **Pipeline** (each stage a pure function in its own `src/` module, with a matching `tests/` file):
  `analyze.ts` (orchestration, ignore set) → `frontmatter.ts` (split/normalize/validate, incl. `projectJsonSafe` cycle-breaking at the yaml.parse boundary) → `markdown.ts` (hand-rolled line scanner: headings, link targets, inline code) + `body.ts` (lines/headings/token budgets) + `tokenizer.ts` (pluggable, default ≈chars/4) → `docs.ts` (README + LICENSE detection, two-tier SPDX) → `manifest.ts` (WebCrypto sha256/size/kind/isText per file) → `references.ts` (declared/resolved/broken/external/orphans; word-boundary matching; orphan check is transitive through ALL text files reachable from SKILL.md — docs and source alike, incl. python/js import forms and the `__init__.py` package rule; declared/resolved/broken stay direct) → `digest.ts` (R1 version-stripped canonical-JSON content hash) → `finalize.ts` (applies `options.rules` severity overrides, computes `ok`, sorts, assembles).
- **`diagnostics.ts`** is the code registry (22 codes; code → default severity/message/hint) + collector. New codes are registered there and must ship with an emitting test fixture in the same commit (a meta-test enforces both directions).
- **CLI**: the `asa` bin (`asa <folder> [--json]`, folder required — bare `asa` prints usage; a folder without SKILL.md is rejected up front as not-a-skill-folder, CLI policy only, `analyze()` still accepts it; exit codes 0 ok / 1 not-ok / 2 usage-or-IO). All logic lives in `src/cli.ts` (`runCli(argv, io)` with injectable IO — unit-testable to 100%); `src/cli-entry.ts` is the logic-free shebang wrapper (coverage-excluded like the barrel). The pretty view is presentation only — `--json` prints the untouched contract.
